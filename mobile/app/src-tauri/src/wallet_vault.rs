...
pub fn import_profile_mnemonic(
    ...
    Mnemonic::new(phrase, kaspa_bip32::Language::English).map_err(|e| e.to_string())?;
...
pub fn unlocked_private_key_hex_for_path(network: &str, derivation_path: &str) -> Result<String, String> {
    let keypair = derive_keypair_for_path(network, derivation_path)?;
    let sk = keypair.secret_key();
    Ok(bytes_to_lower_hex(&sk.secret_bytes()))
}
