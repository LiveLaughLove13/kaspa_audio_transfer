...
#[doc(hidden)]
#[cfg(target_os = "android")]
pub static CASTERS: Lazy<std::sync::Mutex<Vec<fn() -> (TypeId, BoxedCaster)>>> =
    Lazy::new(|| std::sync::Mutex::new(Vec::new()));

#[doc(hidden)]
#[cfg(target_os = "android")]
pub fn __android_register_caster(f: fn() -> (TypeId, BoxedCaster)) {
    CASTERS
        .lock()
        .expect("intertrait CASTERS mutex poisoned")
        .push(f);
}
