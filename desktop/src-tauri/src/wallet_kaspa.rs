...
pub async fn wallet_send_kas(
    ...
    // Match the pattern used elsewhere in the project: use a placeholder signature
    // to compute contextual mass, then set tx mass before signing.
    ...
    // Prefer the computed value; fall back to estimate if needed.
    let mass_to_set = if ctx.storage_mass > 0 {
        ctx.storage_mass
    } else {
        storage_mass
    };
